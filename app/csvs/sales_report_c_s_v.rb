class SalesReportCSV
  ...
  def write_csv filename
    CSV.open(File.join(Rails.root, "tmp", filename), "wb") do |csv|
      csv << ["Product Code", "Design", "Style", "Units Sold", "Price", "Royalty %", "Return $", "Discount $", "Total Sale", "Royalty Due"]
  ...
  def sales_total_for_product product, csv
    ...
    csv << [product.code, product.design.name, product.body_style.name, quantity, ActionController::Base.helpers.number_to_currency(product.price / 100.0), '10.00%', '', '', ActionController::Base.helpers.number_to_currency(gross), ActionController::Base.helpers.number_to_currency(gross / 10.0)]
  end

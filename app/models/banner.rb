class Banner < ActiveRecord::Base
  ...
  scope :in_gallery, lambda { where(:active_in_gallery => true) }
  scope :ordered, lambda { order(:gallery_position) }
  scope :not_from_customers, lambda { where(:customer_uploaded => nil) }
  scope :from_customers, lambda { where(:customer_uploaded => true) }
